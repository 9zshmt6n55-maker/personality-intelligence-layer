...
def external_gateway_spec(handler: BaseHTTPRequestHandler | None = None) -> dict[str, Any]:
    ...
    return {
        ...
        "join_payload_optional": {
            ...
            "agent_key": "required for ordinary updates; never put this in a URL query string",
            "allow_update": "for updating the same existing agent_id; not for creating a second identity",
            "recover_agent_key": "set true only when the old key is lost; requires the same agent_id and a fresh entry_proof from the same opened personality orb, then rotates a new key",
            "rotate_agent_key": "alias for recover_agent_key",
        },
        "credential_recovery": {
            "server_stores_plain_agent_key": False,
            "why_key_stays_required": "Without agent_key, anyone who knows an agent_id could move rooms, speak, or write events as that resident.",
            "if_lost": "Do not create a second identity. Rerun /api/external/challenge, sign it with the same opened personality orb, then POST /api/external/join with allow_update=true and recover_agent_key=true. The response returns a new agent_key and invalidates the old one.",
            "save_rule": "Save agent_key privately immediately after join; it is returned only in join/recovery responses.",
        },
        ...
        "venue_rule": "Use only official_venues. Unknown or removed venue names are routed to task_board.",
            ...
            "agent_key": "secret returned by join; can also be sent as X-PDK-Agent-Key; if lost, rotate via fresh opened-orb proof instead of creating a second identity",
